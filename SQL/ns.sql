...
spool may-june23_detail.txt

select 'Dialed Number,Date,Orig Time,Term Time,Call Duration,OCN,CPM,Band/Tier,Amount Billed' from dual;


select a.chinook_id,'('|| substr(a.dialedno,1,3) || ') ' || substr(a.dialedno,4,3)||'-'||substr(a.dialedno,7,4) "Dialed Number",
orig_dt "Date",
...
from chinook_june a, chinook_bill_june b, intrastate_rate_sheet c
where a.chinook_id=b.chinook_id
and a.call_area = 1
and a.prcmp_id != 1
and to_number(a.class_type)=c.class_type
group by a.chinook_id,'('|| substr(a.dialedno,1,3) || ') ' || substr(a.dialedno,4,3)||'-'|| substr(a.dialedno,7,4),
...
a.class_type,c.cpm
union
select a.chinook_id,'('|| substr(a.dialedno,1,3) || ') ' || substr(a.dialedno,4,3)||'-'||
...
from chinook_june a, chinook_bill_june b, interstate_rate_sheet c
where a.chinook_id=b.chinook_id
and a.call_area = 2
and a.prcmp_id != 1
and to_number(a.class_type)=c.class_type
...
from chinook_june a, chinook_bill_june b
where a.chinook_id=b.chinook_id
...
union
select a.chinook_id,a.dialedno "Dialed Number",orig_dt "Date",
substr(a.orig_time,1,2)||':'||substr(a.orig_time,3,2)||':'||substr(a.orig_time,5,2) "Orig Time",
substr(a.discn_time,1,2)||':'||substr(a.discn_time,3,2)||':'||substr(a.discn_time,5,2) "Term Time",
to_number(a.call_duration_minutes) ||':'||a.call_duration_seconds "Call Duration",
null "OCN",
...
group by a.chinook_id,a.dialedno,
orig_dt, substr(a.orig_time,1,2)||':'||substr(a.orig_time,3,2)||':'||substr(a.orig_time,5,2),
substr(a.discn_time,1,2)||':'||substr(a.discn_time,3,2)||':'||substr(a.discn_time,5,2),
to_number(a.call_duration_minutes) ||':'||a.call_duration_seconds,
...
spool may-june23_summary.txt

select 'Total Billed May-June 23:' from dual; 
select sum("Charge") charges from chinook_bill_june;

select 'NPA/NXX,Total MOU,OCN,Band/Tier' from dual;

select substr(dialedno,1,6) "NPA/NXX", 
trunc(sum(a.call_duration_minutes) + (sum(a.call_duration_seconds)/60)) ||':'||
decode(mod(sum(a.call_duration_seconds),60),'6','06','0','00',mod(sum(a.call_duration_seconds),60)) "TOTAL MOU",
