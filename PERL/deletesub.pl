...
($usr,$passwd,$sid)=&serv_info($test);
	...
	if($is_mac eq "1")
	{
		...
		if(@r2 = $sth2->fetchrow_array)
        	{
				$sth_update = $invh->prepare("update alopa_removal set status='checking cable prov', txt='Checking cableprovs for mac', last_update=NOW() where action='mac' and value=LOWER('$mac')");
				$sth_update->execute() || die "Cannot establish connection to the database";
				$mac = $r2[0];
				$mta = $r2[1];
						remove_from_cableprov($mac, $mta);
				$sth_update = $invh->prepare("update alopa_removal set status='finished', txt='Modem has been removed from the cable prov', last_update=NOW() where action='mac' and value=LOWER('$mac')");
				$sth_update->execute() || die "Cannot establish connection to the database"; 
				...
				print "setting deletestatus=1 on table subs_service_to_device for hsd service $mac\n" if($full and $delete);
							$sth5=$dbh->prepare(q{update  subs_service_to_device set deletestatus = 1 where exists (select 1 from hsd where macaddressofcm=? and hsd.recordnumber=service_record_number) and service_name='HSD'});
							$sth5->execute($mac) if($delete);
				...
				print "setting deletestatus=1 on table subs_service_to_device for voice service $mac\n" if($full and $delete);
							$sth7=$dbh->prepare(q{update  subs_service_to_device set deletestatus = 1 where exists (select 1 from voice where macaddressofcm=? and voice.recordnumber=service_record_number) and service_name='Voice'});
							$sth7->execute($mac) if($delete);
				...
				print "setting deletestatus=1 on table subs_service_to_device for endpoint service $mac\n" if($full and $delete);
							$sth8=$dbh->prepare(q{update  subs_service_to_device set deletestatus = 1 where exists (select 1 from endpoint where macaddressofmta=? and endpoint.recordnumber=service_record_number) and service_name='EndPoint'});
							$sth8->execute($mta) if($delete);

			}
			else
			{
					print "could not find mac address\n";
			$sth_update = $invh->prepare("update alopa_removal set status='failed', txt='Unable to find modem within the BIS database', last_update=NOW() where action='mac' and value=LOWER('$mac')");
                	$sth_update->execute() || die "Cannot establish connection to the database"; 
            }	
	}
	else
	{
		$sth_update = $invh->prepare("update alopa_removal set status='searching db', txt='searching alopa db for account', last_update=NOW() where action='account' and value=LOWER('$acctnum')");
                $sth_update->execute() || die "Cannot establish connection to the database";
		print "Starting search for account\n" if($full);
        	...
        	while ($sth->fetch)
                {
			$sth_update = $invh->prepare("update alopa_removal set status='found account', txt='account was found in alopa db', last_update=NOW() where action='account' and value=LOWER('$acctnum')");
                	$sth_update->execute() || die "Cannot establish connection to the database";
			print "found account\n" if($full);
			...
			while ($sth3->fetch)
                	{
				$sth_update = $invh->prepare("update alopa_removal set status='checking cableprov', txt='checking cable prov', last_update=NOW() where action='account' and value=LOWER('$acctnum')");
                		$sth_update->execute() || die "Cannot establish connection to the database"; 
				print "found mac\n" if($full);
			...
			print "setting subscriptionstatus='deleted' on table subscriber\n" if($full and $delete);
			$sth10=$dbh->prepare(q{update subscriber set subscriptionstatus='deleted' where recordnumber=?});
	        	$sth10->execute($parentref) if($delete);
			
			print "$acctnum Deleted from Alopa Database\n" if($delete);
		
                }
		$sth_update = $invh->prepare("update alopa_removal set status='finished', txt='account removed', last_update=NOW() where action='account' and value=LOWER('$acctnum')");
                $sth_update->execute() || die "Cannot establish connection to the database";
	}
