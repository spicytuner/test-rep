...
$test='3';
$dbh2 = DBI->connect("dbi:mysql:database=Inventory:$sid", $nmsuser, $nmspass) or die "Can't connect to nms2!";
$sth9=$dbh2=prepare(q{ select cm_mac, cmts_name from cm inner join cmts on cm.cm_id = cmts.cmts_id});
$sth9->execute();
$sth9->bind_columns({}, \($cmmac,$cmts));

$sth10=$dbh=prepare(q{update cust set cmts=? where cmmac=?});
$sth10->execute($cmts,$cmmac);
