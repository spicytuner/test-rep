...
open(FILE,">/home/jsoria/SCRIPTS/PERL/cust_remedy.dat");

$sth=$dbh2->prepare(q{select accountnumber,node,firstname,lastname,phone,street1,street2,city,state,
zip,last_changed
from cust 
where accountnumber not in (select accountnumber from customer_info)});    
$sth->execute();
