...
$sth5=$dbh->prepare(q{truncate table cust_remedy });
$sth5->execute();
...
from cust_import@samprd a, pi b, hsd c, subscriberproxy d, subscriber e,
endpoint f, locationtree g
where a.subscriberkey=b.userid(+)
and a.subscriberkey like '8313%'
...
$sth7=$dbh->prepare(q{insert into cust_remedy
select subscriberkey, node, firstname, lastname, phone, address1, address2, city, state, zip,
'','Video','','','','','','',lastchangeddate,video,data,bdp,''
from cust_import@samprd
where subscriberkey like '8313%'
and video=1
});
$sth7->execute();
